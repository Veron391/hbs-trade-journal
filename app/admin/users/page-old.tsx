...
import { useRecentUsers } from '../../../lib/hooks/useAdminData';
...
export default function UsersPage() {
  ...
  // Use real API data
  const { data: recentUsers, isLoading: usersLoading } = useRecentUsers(50); // Get more users for the table

  // Calculate recent users count 
  ...
  const processedUsers = useMemo(() => {
    ...
    return recentUsers.map(user => ({
      ...
      totalTrades: user.tradesCount,
      totalPL: 0, // Will be calculated when we add user trades aggregation
      winRate: 0, // Will be calculated when we add user trades aggregation
      avgTradeSize: 0, // Will be calculated when we add user trades aggregation
      riskScore: 'Low' as const, // Will be calculated when we add risk analysis
      lastTradeDate: user.lastLoginAt || user.createdAt,
  ...
  useEffect(() => {
    ...
    const filteredUsers = allUsers.map(user => {
      ...
      // For new users (21-25), they should always be inactive regardless of trades
      const isNewUser = user.id >= 21;
  ...
  }, [period, category, range, searchQuery, statusFilter]);
